# self-healing.bash
# This script is sourced in the user's .bashrc or .bash_profile to enable Autonomous Self-Healing CLI integration.
...
self_healing_hook_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)"

# Preexec hook: Saves the command that is about to run
self_healing_preexec() {
  # If the flag is not set, skip the logic
  [ -n "$self_healing_on" ] || return
  
  # Clear the flag to prevent recursive calls for sub-commands or prompts
  self_healing_on=""
  
  # Write the command to last cmd file
  printf "%s\n" "$BASH_COMMAND" > "$LAST_CMD_FILE"
}
...
self_healing_precmd() {
  ...
  # Turn preexec trapping back on for the next command
  self_healing_on=1
}

# Set up trap and PROMPT_COMMAND
trap 'self_healing_preexec' DEBUG

if [[ -z "$PROMPT_COMMAND" ]]; then
  PROMPT_COMMAND="self_healing_precmd"
elif [[ "$PROMPT_COMMAND" != *"self_healing_precmd"* ]]; then
  # If PROMPT_COMMAND contains multiple commands, prepend ours to capture exit code first
  PROMPT_COMMAND="self_healing_precmd; $PROMPT_COMMAND"
fi
